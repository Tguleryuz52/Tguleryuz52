#!/usr/bin/env python3
"""
Generate a clean, centered monochrome ASCII portrait SVG with typewriter effect.
"""
import html
import os
import sys
...
GAMMA = 1.15
WHITE_FLOOR = 0.70  # anything brighter than this is pure space (guarantees empty background)
...
def main():
    ...
    svg_parts.append(f'''
        ...
        .ascii-text {{ font-family: ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, monospace; font-size: 7.6px; fill: {INK}; letter-spacing: 0.6px; }}
    ''')
    svg_parts.append('</style>')
    ...
    for i, line in enumerate(rows_txt):
        ...
        svg_parts.append(f'  <rect x="18" y="{y_pos - 7.5}" width="0" height="{line_h + 1}">')
        svg_parts.append(f'    <animate attributeName="width" from="0" to="{CANVAS_W - 36}" dur="{ROW_DUR}s" begin="{delay:.2f}s" fill="freeze" />')
        svg_parts.append('  </rect>')
        svg_parts.append('</clipPath>')
        
        svg_parts.append(f'<text class="ascii-text" x="18" y="{y_pos}" clip-path="url(#{clip_id})">{escaped}</text>')
    ...
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("\n".join(svg_parts))
    print(f"Generated clean ASCII SVG: {OUT}")
