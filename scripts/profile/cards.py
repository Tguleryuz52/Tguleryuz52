...
def stats_card(d, theme, W=500, H=195):
    ...
    rows = [("star", "Total Stars Earned:", st["stars"]), ("history", "Total Commits:", st["commits"]),
            ("pr", "Total PRs:", st["prs"]), ("issue", "Total Issues:", st["issues"]),
            ("repo", "Contributed to (last year):", st["contributed_to"])]
    o = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" '
    ...
    for i, (icon, label, val) in enumerate(rows):
        ...
        o.append(f'<g transform="translate(25,{y})"><g {anim("rise", 0.45 + 0.15 * i, 0.4)}>'
                 ...
                 f'<text x="230" y="12.5" font-size="14" font-weight="600" fill="{c["text"]}">{val:,}</text></g></g>')
    o.append("</svg>")
    return "\n".join(o)
